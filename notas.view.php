...
<body>    
<!-- ----------------------PRIMER RECUADRO NOTAS.VIEW---------------------------------------- -->
<!-------------------------------- card-header --------------------------------------->
<div class="conteiner m-2 mt-3 " >
                    ...
                    if(!isset($_GET['revisar'])){
                        ...
                        <form method="get"  action="notas.view.php">  
                            <ul class="list-group list-group-flush">
                                ...
                                <!-- ----------------------------------------------------------- -->
                                <li class="list-group-item">
                                        <label class="font-weight-bold">Seleccione el Ciclo Lectivo</label><br>
                                ...
                                <!-- ----------------------------------------------------------- -->
                                <li class="list-group-item">
                                        <label class="font-weight-bold">Seleccione la Sección</label><br>
                    ...
                    <form action="procesarnota.php" method="post">
                        <table class="table table-bordered table-striped" >
                            ...
                            <?php foreach ($alumnos as $index => $alumno) :?>
                                ...
                                <!-- ---------------------------------------------------------------------------- -->
                                <tr>
                                    <td scope="row"><?php echo $alumno['num_lista'] ?></td>
                                    ...
                                    if(existeNota($alumno['id'],$id_materia,$conn) > 0){
                                            ...
                                            foreach ($registrosnotas as $eval => $nota){
                                                ...
                                                echo '<td><input type="text" class="form-control"  maxlength="5" value="'.$nota['nota'].'" name="evaluacion' . $eval . 'alumno' . $index . '" class="txtnota"></td>';
                                            }
                                            if($num_eval > $num_notas){
                                                ...
                                                for($i = $num_notas; $i < $dif + $num_notas; $i++) {
                                                    ...
                                                    echo '<td><input type="text" class="form-control" maxlength="5" value="'.$nota['nota'].'" name="evaluacion' . $i . 'alumno' . $index . '" class="txtnota"></td>';
                                                }
                                            }
                                        }else {
                                            //extrayendo el numero de evaluaciones para esa materia seleccionada
                                            for($i = 0; $i < $num_eval; $i++) {
                                                echo '<td><input type="text" class="form-control" maxlength="5" name="evaluacion' . $i . 'alumno' . $index . '" class="txtnota"></td>';
                                            }
                                        }
                                        echo '<td align="center">'.number_format($alumno['promedio'], 2).'</td>';
                                        if(existeNota($alumno['id'],$id_materia,$conn) > 0){
                                            echo '<td><input type="text" class="form-control" maxlength="100" value="'.$alumno['observaciones'].'" name="observaciones' . $index . '" class="txtnota"></td>';
                                        }else {
                                            echo '<td><input type="text" class="form-control" name="observaciones' . $index . '" class="txtnota"></td>';
                                        }
                                    if(existeNota($alumno['id'],$id_materia,$conn) > 0){
                                        echo '<td><button  class="btn btn-danger" href="notadelete.php?idalumno='.$alumno['id'].'&idmateria='.$id_materia.'">Eliminar</button> </td>';
